...
| `python -c "코드"` | `-c` = **c**ommand | 파일 없이 코드 바로 실행 |
| `pip` | **P**ip **I**nstalls **P**ackages (재귀 약자) | 파이썬 패키지 설치 |
| `pip install torch --index-url URL` | index-url = 창고 주소 | 특정 CUDA 버전용 PyTorch 설치 |
...
| 명령 | 풀네임 / 유래 | 뜻 |
|------|--------------|-----|
| `nvidia-smi` | NVIDIA **S**ystem **M**anagement **I**nterface | GPU 상태·이름·VRAM·온도 확인 |

**핵심 용어**
- **CUDA** = NVIDIA GPU한테 계산 시키는 기술 (`torch.cuda.is_available()`이 `True`면 준비 끝)
- **VRAM** = GPU 전용 메모리. 모델 크기 한계를 정함
- **fp16** = 숫자 16비트로 저장 → 메모리 절반, 속도↑ (`.half()`)
- **Tensor Core** = 행렬곱 전용 초고속 부품 (fp16일 때 발동)

**VRAM으로 모델 크기 어림잡기** (fp16 기준):
- 규칙: 파라미터 1개 = 2바이트
- `VRAM(GB) ÷ 2` = 실을 수 있는 파라미터 수(십억 단위)
- 예: 12.8GB ÷ 2 ≈ 6.4B → 실사용은 절반(≈3B 추론), 훈련은 1B 이하가 현실적
...
| `cat > 파일 << EOF` ... `EOF` | heredoc | 두 EOF 사이 내용을 파일로 저장 (여러 줄 한 번에) |
| `_` (파이썬) | 밑줄 | "안 쓸 값" 버릴 때 쓰는 변수 이름 |
